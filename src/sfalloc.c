...
int main(int argc, char *argv[]) {
    ...
    *value2 = VALUE2_VALUE;
    ...
    printf("=== LETS PRINT THE EXPLICIT LIST ===\n");
    sf_free_header* hh = freelist_head;
    while(hh != NULL)
    {
	    sf_blockprint(hh);
	    hh = hh->next;
    ...
    // test splinter
    printf("=== Test8: Splinter handling ===\n");
    void * m1 =sf_malloc(32);
    sf_varprint(m1);
    void *m2 = sf_malloc(32);
    sf_malloc(32);
    sf_malloc(32);
    printf("m2 varprint\n");
    sf_varprint(m2);
    sf_free(m2);
    printf("freed m2 varprint\n");
    sf_varprint(m2);
    void *m2_al = sf_malloc(4);
    printf("m2 varprint\n");
    sf_varprint(m2_al);
    press_to_cont();
